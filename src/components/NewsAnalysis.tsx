    ...
    try {
      ...
      if (data.error) {
        toast({
          ...
          description: `News data retrieved, but ${data.error}`,
          variant: "default",
        });
      } else {
        toast({
          title: "Analysis Complete",
          description: `Successfully analyzed news and price data for ${companyName}`,
        });
      }
    } catch (error: unknown) {
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : "Failed to analyze news. Please check if the backend server is running or try again later.";
                ...
                <Card className="bg-card border-border">
                  <CardHeader>
                    <CardTitle className="text-foreground">
                      {result.stock_name} Stock Overview
                    </CardTitle>
                    <CardDescription className="text-muted-foreground">
                  ...
                  <CardContent>
                    <div className="grid md:grid-cols-3 gap-6">
                      <div className="text-center p-4 bg-muted rounded-lg">
                        ...
                        <p className="text-2xl font-bold text-foreground">
                          {result.current_price === null
                            ? "N/A"
                            : `$${safeToFixed(result.current_price)}`}
                        </p>
                        {result.timestamp && (
                          <p className="text-muted-foreground text-xs mt-1">
                            As of {result.timestamp} (delayed)
                          </p>
                        )}
                      </div>
                      ...
                      <div className="text-center p-4 bg-muted rounded-lg">
                        ...
                        >
                          {result.change_percent === null
                            ? "N/A"
                            : `${result.change_percent >= 0 ? "+" : ""}${safeToFixed(
                    ...
                    <p className="text-muted-foreground text-sm mt-4">
                      * Prices are based on delayed data from yfinance and may not reflect real-time values shown in the chart.
                    </p>
                  </CardContent>
                </Card>
              )}
              {result.news && result.news.length > 0 && (
                <Card className="bg-card border-border">
                  <CardHeader>
                    ...
                    <CardDescription className="text-muted-foreground">
                      Top 5 news articles from NewsAPI
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                      {result.news.map((item, index) => (
                        <div
                          key={index}
                          className="border-l-4 border-brand-accent/30 pl-4 py-3 bg-muted rounded-r-lg"
                        >
                          <a
                            href={item.url}
                            target="_blank"
